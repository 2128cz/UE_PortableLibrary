...
对于各个想要运行的摄像机来说，有优先级和相关性两个等级约束；  
* 优先级将允许高优先的摄像机视图优先使用；  
* 相关性则允许同一个场景下有多个视图规则，且不同相关性的规则互不干扰；

摄像机对象的基本运行说需要配合`跟随视图管理组件`，放置在被控制的角色身上，即可将视图自动转移到目标摄像机上；  
但想要让摄像机能够被指定，需要先让摄像机指向角色，并在开始时请求角色控制；此时如果有多个摄像同时请求控制，
则会按照相关性下的优先级进行排除，同一优先级下距离最近的摄像机进行`允许控制回调`，
如果在这之前拥有已经处于启动状态的摄像机，那么则会再产生一个`关闭回调`。  
...
管理器中自带几种基础的重载器：
* **静态场景焦点**  
放置在场景中，不允许移动，通常情况下都应该优先考虑静态场景焦点  
本身并无约束方法，但会提供一个权重焦点，在用户设定的权重下将按比例将摄像机向体积偏移。  
默认优先级5（平衡）


* **动态对象焦点**  
放置在场景中，本身不允许移动，但允许将指定目标的优先级作为自身优先级加入到焦点计算中  
对突发事件，需要注意的动画事件等有独特的焦点转移方法  
本身并无约束方法，也不会提供新的摄像机吸附，优先级也十分低，但如果遇到指定类的对象时，则会将对象的优先级赋予自己。  
默认优先级8（极低）


* **路径专注焦点**  
放置在场景中，不允许移动，如有时应当优先考虑此体积焦点，具有一个样条驱动的路径，摄像机进入时几乎由样条曲线约束。  
默认优先级3（极高）


* **状态重载体积**   
放置在场景中，原则上允许移动，仅重设参数，方法类似进出`PostProcessVolume`时的后处理效果。   
默认优先级4（高）


* **摄像机强制重载体积**   
放置在场景中，原则上允许移动；可以将角色视图强制重载到新的视图中，这通常使用在需要将视角切换到一个具体物体上时  
也可以选择使用序列编辑器来作为新视图驱动，当序列播放完毕，或是定时结束时，将返还视图目标，除非这个视图目标指定了角色。  
默认优先级3（较高）
...
7. 较低
8. 极低  
默认值不会低于2，为了给用户操作留空间；当然字节的最大值是255,，你可以通过重载来实现更多的等级。
&nbsp;  
